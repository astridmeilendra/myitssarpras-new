    ...
    <div class="relative h-full">
        ...
        <div class="flex flex-col h-full overflow-y-auto" style="scrollbar-width: none; -ms-overflow-style: none;">
            ...
            <div class="mb-6">
                @php
                    // Ambil foto dari database (string dengan koma)
                    $images = [];
                ...
                <div class="flex gap-2 w-full">
                    @foreach($images as $index => $img)
                        <button
                            type="button"
                            class="thumbnail-item w-full h-16 rounded-lg overflow-hidden
                                {{ $index === 0 ? 'border-1 border-[#003D82]' : 'border border-gray-200' }}"
                            data-large-src="{{ $img }}">
            ...
            <div class="modal-content bg-white w-full shadow-2xl overflow-hidden rounded-t-3xl flex flex-col" style="max-height: 85%;">
                ...
                <form id="bookingForm" class="flex-1 px-6 pt-8 pb-10 flex flex-col justify-center" enctype="multipart/form-data">
                    ...
                    <div class="mt-6">
                        <button type="submit"
                                id="submitBtn"
                                class="w-full bg-[#003D82] text-white py-3 rounded-lg font-medium hover:bg-[#002d61] transition-colors">
                            Kirim
                        </button>
                    </div>
                </form>
        ...
        ['tanggal', 'waktu'].forEach(field => {
            document.getElementById(field).addEventListener('change', function() {
                ...
                checkTimeout = setTimeout(async () => {
                    ...
                    if (tanggal && waktu) {
                        try {
                            const response = await fetch('{{ route("peminjaman.check-availability") }}', {
                                method: 'POST',
                                headers: {
                            ...
                            if (result.success && !result.available) {
                                alert('Ruangan sudah dipinjam pada tanggal dan waktu tersebut. Silakan pilih waktu lain.');
